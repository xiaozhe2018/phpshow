...
function debug_hanlde_xhprof()
{
    if(PHP_SAPI !== 'cli' && function_exists('xhprof_enable') && DEBUG_MODE === true)
    {
        ...
        echo "<center><a href='/bone/core/library/debug/xhprof_html/index.php?run=$run_id&source=xhprof_foo'>xhprof性能日志</a></center>";
    }
}
...
function debug_format_errmsg($log_type='debug', $errno, $errmsg, $filename, $linenum, $vars)
{
    ...
    if( empty($_debug_errortype[$errno]) )
    {
        $_debug_errortype[$errno] = "<font color='#466820'>手动抛出</font>";
    }
    ...
    $err .= "错误类型：" . $_debug_errortype[$errno] . "<br />\n";
...
{
    global $_debug_error_msg, $_debug_hidden, $_debug_safe_ip;
    if( $_debug_error_msg != '' )
    {
        if( (DEBUG_MODE === true || $_debug_safe_ip===true) && !$_debug_hidden )
        {
            ...
            echo '<div style="line-height:24px; background: #FBFEEF;;"><div style="float:left"><strong>YLMF框架应用错误/警告信息追踪：</strong></div><div style="float:right"><a href="#" onclick="javascript:debug_close_all();" target="_self">[关闭全部]</a></div>';
