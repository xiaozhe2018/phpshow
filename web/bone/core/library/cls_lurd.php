...
class cls_lurd
{
    ...
    */
	public function set_table( $tablename )
	{
		...
		if( preg_match("/\./", $tablename) ) {
		    $dbs = explode('.', $tablename);
		    db::select_db( $dbs[0] );
		    $this->table_name = $dbs[1];
		    $this->db_name = $dbs[0];
		}
	...
	*
	* @parem $condition  手动指定更新条件(key_value_array指定了主键或唯一索引值，此选项可省略)
	*
	* @parem $tablename  如果已经用 set_table 指定，这个参数可以省略
	*
	* @return int result
	*/
	public function update( $key_value_array, $condition='', $tablename='' )
	{
		...
		$editfield = '';
		$primary_keys = explode(',', $this->primary_key);
		$check_lock_field = (count($this->lock_fields) > 0 ? true : false);
		foreach($this->fields as $k => $v)
		{
			//不更新主键和自动产递增的键(如果是复合主键，允许更新相关值)
			if(!isset($key_value_array[$k]) || $k == $this->auto_field || $k == $this->primary_key ) {
				continue;
			}
			//不处理没指定的字段
			if( $check_lock_field && !isset($this->lock_fields[$k]) ) {
		...
		if( $condition=='' )
		{
			...
			if( preg_match('/,/', $this->primary_key) )
			{
				...
				$keyvalue = $this->_filter_data($this->primary_key, $key_value_array[$this->primary_key]);
			}
			$keyvalue = preg_replace("/[^\w]/", '', $keyvalue);
			if( $keyvalue=='' ) {
				throw new Exception("cls_lurd-&gt;update Update error -- empty primary key value and empty condition! ");
			}
			$condition = ( !$isjoin ? " `{$this->primary_key}`='{$keyvalue}' " : " MD5( CONCAT(`".str_replace(',', '`,`', $this->primary_key)."`) ) = '{$keyvalue}' " );
		}
		else
		{
		...
		if( !preg_match("/(=|in\()/", $condition) ) {
		    throw new Exception("cls_lurd-&gt;update 条件太广泛，为了避免操作错误，请使用db类进行操作");
		}
    ...
    */
	public function get_datas( $pageno=1, $listfield = '*', $orderquery ='', $wherequery='', $tablename='', $count_row=true )
	{
		...
		else
		{
		    ...
		    if( !preg_match('/^where[\s]/i', $wherequery) ) {
		...
		//$row = db::get_one(' SELECT FOUND_ROWS() as dd ', DB_GET_ASSOC, $this->query_master);
...
/**
    ...
    * @return string
    */
	public function get_pagination( $url = '' )
	{
	    $config = array('count_num' => $this->total_result, 
	                    'pagesize' => $this->page_size, 
	                    'pagename' => 'pageno', 
	                    'cur_page' => $this->page_no,
	                    'css_class' => 'lurd-pager');
	    return db::pagination( $url, $config );
	}
    ...
    * @return void
    */
	protected function _analyse_table()
	{
		if($this->table_name == '')
		{
			exit(" No Input Table! ");
		...
		$infos_file = 'table_infos_'.$this->db_name.$this->table_name;
		...
		for($i=1; $i < count($flines)-1; $i++ )
		{
			...
			//字段名称、类型
			$this->fields[$lines[0]] = array('type' => '',  'length' => '', 'unsigned' => false, 'autokey' => false, 'null' => true,
			                                 'default' => '', 'em' => '', 'comment'=>'', 'lurd_type' => '');
		...
		$tb_infos['primary_key'] = $this->primary_key;
		cache::set('lurd_', $infos_file, $tb_infos);
	...
	* @return string
	*/
	protected function _filter_data($fname, $fvalue)
	{
		$revalue = '';
		...
		else if(in_array($ftype, $this->date_types))
		{
			if($ftype=='TIMESTAMP')
			{
				$revalue = self::get_mktime($fvalue);
			}
			else
			{
				$revalue = preg_replace("/[^0-9 :-]/", '', $fvalue);
	...
	* 对插入数据库的字符串禁用安全处理
	*/
	public function set_safe_check( $step = 1 )
	{
	    $this->string_safe = $step;
	    if( $step==0 ) {
	        db::$safe_test = false;
	    }
	}
