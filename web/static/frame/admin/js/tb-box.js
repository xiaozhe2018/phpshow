...
var tb_pathToImage = "/static/frame/admin/images/loading.gif";
...
function tb_show_loading()
{
    ...
    var dstyle = '';
    if( tb_offset ) {
        dstyle = " style='position:absolute; left:"+tb_offset.left+"px; top:"+tb_offset.top+"px' ";
    }
    $("body").append("<div id='TB_load'"+dstyle+"><img src='"+imgLoader.src+"' /></div>");
    $('#TB_load').show();
}
...
function tb_showmsg(msg, caption, talign, ww, wh)
{
		...
		$("#TB_window").css({display:"block"}).css({top:"120px"}).css({'margin-left':'-'+Math.ceil($("#TB_window").width()/2)+"px"});
		//alert(ww);
...
function tb_position()
{
	if( tb_type==0 )
	{
	    $("#TB_window").css({marginLeft: '-'+(TB_WIDTH-220)+'px', width: TB_WIDTH + 'px'});
	    //$("#TB_window").css({left: '100px'});
	    // take away IE6
